...
def build_default_config(action_dim: int) -> WMConfig:
    ...
    # Success-only training: no reward learning
...
def train_one_task(
    ...
    # target hw
    target_hw = target_hw_override if target_hw_override is not None else choose_target_hw_from_raw(raw_h, raw_w, task_name)
    ...
    # Train loop
    t0 = time.time()
    for it in trange(1, train_steps + 1, desc=f"TRAIN {task_name} seed={seed}"):
        ...
        if it % log_every == 0:
            ...
            print(f"[{task_name}][seed={seed}][{it:6d}] decoder={dec_loss:.4f} reward={r_loss:.4f} cont={c_loss:.4f} kl={kl:.4f} ({dt:.1f}s)")
            t0 = time.time()

    # Save under per-seed folder
    out_dir = os.path.join(out_root, task_name, f"seed_{seed}")
...
def main():
    ...
    ap.add_argument("--out_root", type=str, default="/home/s447658/projects/dreamer_fiper_offline/wm_all5_seeds")
    ...
    ap.add_argument("--target_hw_pretzel", type=str, default=None)
    ...
    view_modes = {"sorting": args.view_mode_sort, "stacking": args.view_mode_stack}
