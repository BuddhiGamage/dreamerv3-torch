...
Key upgrades (vs v2):
- EPISODE-LEVEL conformal calibration (thr calibrated on per-episode score).
- Optional success pool source:
    * calib_only (strict, protocol-safe)
    * calib_plus_test_success (NOT protocol-safe; provided for ablations only)
- NEW: per-task bins selection:
    * --bins can be an int (fixed for all tasks), or "auto" (chosen per-task from calib successes only)
    * optional explicit overrides: --bins_per_task "push_chair=4,pretzel=10"
- Extra diagnostics:
...
- Computes TWA (timestep-weighted accuracy) in the spirit of FIPER:
...
Example:
  python wm_progress_monitor_per_task_threshold_episode_calib_v3.py \
    ...
    --bins_per_task "push_chair=4,pretzel=10" \
    ...
    --calib_score_mode topk_mean --debug 1
...
def compute_bin_counts_only(success_feats: List[np.ndarray], B: int) -> np.ndarray:
    """
    Count how many success timesteps fall in each normalized-time bin (no fitting).
    """
...
# ============================================================
# Bins selection helpers
# ============================================================
def parse_bins_per_task(s: str) -> Dict[str, int]:
    """
    Parse "push_chair=4,pretzel=10" -> {"push_chair": 4, "pretzel": 10}
    """
    out: Dict[str, int] = {}
    s = (s or "").strip()
    if not s:
...
def choose_bins_auto_from_calib_success(
    ...
    """
    Choose largest B in [bins_min, bins_max] such that
    min bin count >= min_pts_per_bin using calib success episodes only.

    Returns:
      B_chosen, bin_counts_for_B
    """
    ...
    # if nothing satisfies, return bins_min anyway (best-effort)
...
def parse_args():
    ...
    ap.add_argument("--bins", type=str, default="10",
                    help="Either an integer (fixed bins for all tasks) or 'auto' (per-task bins chosen from calib successes only).")
    ap.add_argument("--bins_min", type=int, default=4, help="Used when --bins=auto")
    ap.add_argument("--bins_max", type=int, default=10, help="Used when --bins=auto")
    ap.add_argument("--min_pts_per_bin", type=int, default=20, help="Used when --bins=auto")
    ap.add_argument("--bins_per_task", type=str, default="",
                    help="Optional explicit per-task override, e.g. 'push_chair=4,pretzel=10'. Overrides --bins/auto.")
    ...
    ap.add_argument("--out_json", type=str, default="",
                    help="If empty, saves to --out_root/wm_progress_monitor_episode_calib_v3.json")
...
def resolve_bins_for_task(
    ...
    """
    Decide bins for this task.
    Priority:
      1) explicit override in bins_per_task
      2) if bins_arg == 'auto' -> choose from calib successes only
      3) else -> int(bins_arg)
    Returns: (B, note, bin_counts_estimate)
    """
...
def main():
    args = parse_args()
    bins_per_task = parse_bins_per_task(args.bins_per_task)
    ...
    if str(args.bins).strip().lower() == "auto" or bins_per_task:
        print(f"[CFG] bins_auto: bins_min={args.bins_min} bins_max={args.bins_max} min_pts_per_bin={args.min_pts_per_bin}")
        if bins_per_task:
            print(f"[CFG] bins_per_task overrides: {bins_per_task}")
    ...
    for task in args.tasks:
        ...
        # Build success pool (for refs + threshold calibration)
        ...
        if args.debug:
            ...
            test_succ_list = [feat for feat, yi in zip(test, y) if int(yi) == 0]
            test_fail_list = [feat for feat, yi in zip(test, y) if int(yi) == 1]
            print(f"[LEN] test_success : {_length_stats(test_succ_list)}")
            print(f"[LEN] test_failure : {_length_stats(test_fail_list)}")

        # Resolve bins per task (AUTO uses calib successes ONLY)
        B_task, bins_note, bin_counts_est = resolve_bins_for_task(
            task=task,
            calib_success_list=list(calib),
            bins_arg=args.bins,
            bins_per_task=bins_per_task,
            bins_min=args.bins_min,
            bins_max=args.bins_max,
            min_pts_per_bin=args.min_pts_per_bin,
        )
        print(f"[BINS] bins_task={B_task} ({bins_note})  calib_success_bin_counts_est={bin_counts_est.tolist()}")

        # Fit refs on success pool using chosen bins
        ...
        for feat, yi in tqdm(list(zip(test, y)), desc=f"{task} TEST", leave=False):
            ...
            # Record detection time only for TRUE POSITIVES
        ...
        per_task_results.append({
            ...
            "metrics": {
                ...
                "tp_det_times_norm": det_times_norm,  # for exact reproducibility
            },
        })
    ...
    out: Dict[str, Any] = {
        "method": "wm_progress_monitor_per_task_single_threshold_episode_calib_v3",
        "config": vars(args),
        "bins_per_task_overrides": bins_per_task,
    ...
    if out_json == "":
        out_json = os.path.join(args.out_root, "wm_progress_monitor_episode_calib_v3.json")
